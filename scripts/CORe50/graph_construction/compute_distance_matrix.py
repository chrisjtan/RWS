...
distance_matrix = row_norm_min_max(D)
for row in range(len(distance_matrix)):
    for col in range(len(distance_matrix)):
        # distance_matrix[row, col] = (max(distance_matrix[row, col], distance_matrix[col, row]) +
        #                              min(distance_matrix[row, col], distance_matrix[col, row])) / 2
        distance_matrix[row, col] = max(distance_matrix[row, col], distance_matrix[col, row])
        # distance_matrix[row, col] = min(distance_matrix[row, col], distance_matrix[col, row])

for row in range(len(distance_matrix)):
    for col in range(len(distance_matrix[row])):
        if row == col:
            distance_matrix[row, col] = -1
best_thread, max_f_score = distance_metric_evaluation(distance_matrix, sequence_category_list, beta=.5)
print('matching f_score: ', max_f_score)

D_neg.dump('D_neg.pickle')
D.dump('D.pickle')
